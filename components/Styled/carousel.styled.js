...
const Arrow = styled.img`
  transform: rotate(
    ${(props) => (props.$direction === "right" ? "180deg" : "none")}
  );
  width: 35px;
  height: 37px;
  cursor: pointer;
...
const NavClick = styled(Link)`
  ...
  transition-duration: ${(props) => {
    if (
      (props.$direction === "left" && props.$index === -1) ||
      (props.$direction === "right" && props.$index === props.$length)
    ) {
      return "none";
    } else {
      return "0.5s";
  ...
  @media (min-width: 768px) {
    ...
    box-shadow: ${(props) =>
      props.$square
        ? "none"
        : "0 0 5px #fff, 0 0 10px #fff, 0 0 15px #fff, 0 0 20px #FF66C4, 0 0 30px #FF66C4, 0 0 40px #FF66C4"};
    left: ${(props) => {
      if (props.$square) {
        return props.$index * 100;
      }

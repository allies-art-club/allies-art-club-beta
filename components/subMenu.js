...
const SubMenu = (props) => {
    ...
    return(
        <SideNavWrapper $submenu={"true"} $visible={props.visible?props.visible:""} $menu={"true"}>
            <SideNavList>
                    {
                        props.navLinks.map((el,i)=>
                            <SideNavListElement 
                                key={i}
                                $allcategory={el.match(/All /)?"true":""}
                            >
                                <NavLinkSide onClick={(e)=>{
                                    ...
                                    href={el.match(/All /)?`/${props.mainCategory}`:`/${props.mainCategory}/${pageNavigate(el)}`}>{el}</NavLinkSide>
                            </SideNavListElement>
                        )
                    }
            </SideNavList>

...
import {handleSubmitSupplies} from '../../../utils/Actions/donateActions';
...
const DonateSupplies=(props)=>{
    ...
    const DonateFundsSchema = yup.object().shape({
        ...
        supplies: yup.array().min(1,'Please select something to donate').required('Please select something to donate')
    })
    return(
        <Fragment>
            <TitleBanner 
...
const mapDispatchToProps=(dispatch)=>{
    return {
        ...
        handleSubmitSupplies: (values,csrf)=>handleSubmitSupplies(values,csrf,dispatch)
    }
}
